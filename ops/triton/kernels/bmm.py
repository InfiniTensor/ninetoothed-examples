...
def kernel(
    ...
    input_ptr_batch = input_ptr + bid * input_stride_b
    other_ptr_batch = other_ptr + bid * other_stride_b
    output_ptr_batch = output_ptr + bid * output_stride_b

    offs_am = (pid_m * BLOCK_SIZE_M + tl.arange(0, BLOCK_SIZE_M)) % m
    offs_bn = (pid_n * BLOCK_SIZE_N + tl.arange(0, BLOCK_SIZE_N)) % n
    offs_k = tl.arange(0, BLOCK_SIZE_K)

    input_ptrs = input_ptr_batch + (
        offs_am[:, None] * input_stride_m + offs_k[None, :] * input_stride_k
    )
    other_ptrs = other_ptr_batch + (
    ...
    output_ptrs = output_ptr_batch + (
        output_stride_m * offs_cm[:, None] + output_stride_n * offs_cn[None, :]
    )

    tl.store(
        output_ptrs, accumulator, mask=(offs_cm[:, None] < m) & (offs_cn[None, :] < n)
    )
