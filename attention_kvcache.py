...
def arrangement(
    ...
    return (
        ...
        arrange_cache_end_or_kv(v),
        arrange_k_or_v(k_cache_start),
        arrange_cache_end_or_kv(k_cache_end),
        arrange_k_or_v(v_cache_start),
        arrange_cache_end_or_kv(v_cache_end),
        arrange_causal_mask(causal_mask),
        arrange_q_or_o(o),
    )
...
q, k, v, k_cache_start, k_cache_end, v_cache_start, v_cache_end, causal_mask, o = (
    Tensor(4, constexpr_shape=True) for _ in range(9)
)
...
def attention(
    q, k, v, k_cache_start, k_cache_end, v_cache_start, v_cache_end, causal_mask
):
...
if __name__ == "__main__":
    ...
    def create_causal_mask(shape):
        ...
        return torch.triu(
            torch.full(mask_shape, float("-inf"), device="cuda"),
            diagonal=1,
        )
    ...
    print(torch.max(torch.abs(ninetoothed_output - triton_output)))
    ...
    @triton.testing.perf_report(
        triton.testing.Benchmark(
            ...
            ylabel="TFLOPS",
            plot_name="attention-kvcache-performance",
            args={},
        )
    )
    def benchmark(seq_len, provider):
        ...
        if provider == "ninetoothed":
            ...
            ms = triton.testing.do_bench(
                lambda: triton_attention(q, complete_k, complete_v)
            )
